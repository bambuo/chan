...
func tryBuildDef(bis []types.MergedBi, start int) (*types.Segment, int) {
	if start+2 >= len(bis) {
		return nil, len(bis)
	}
	...
	s, np := extendDef(s, bis, start+3)
	return &s, np
}

func extendDef(s types.Segment, bis []types.MergedBi, pos int) (types.Segment, int) {
	for pos < len(bis) {
		cur := bis[pos]
